...
from app.tools.financial_tools import StockDataFetcher, PortfolioAnalyzer, MarketAnalyzer
...
def analyze_portfolio_tool(holdings: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        for holding in holdings:
            if not all(key in holding for key in ["symbol", "quantity", "purchase_price"]):
                return {"error": "Each holding must have symbol, quantity, and purchase_price"}
...
def find_investment_opportunities(
    market_cap_range: Optional[tuple] = None,
