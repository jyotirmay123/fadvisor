    ...
    def get_technical_indicators(symbol: str, period: str = "3mo") -> Dict[str, Any]:
        ...
        try:
            ...
            indicators = {
                ...
                "sma_50": ta.trend.sma_indicator(close, window=50).iloc[-1] if len(close) >= 50 else None,
                ...
                # Volume indicators
                "volume_sma": ta.volume.volume_weighted_average_price(high, low, close, volume).iloc[-1],
                ...
                "volatility": close.pct_change().std() * np.sqrt(252) * 100,  # Annualized volatility
...
class PortfolioAnalyzer:
    ...
    def analyze_portfolio(holdings: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": PortfolioAnalyzer._generate_recommendations(portfolio_data, sector_allocation)
        }
    
    @staticmethod
    def _analyze_holding(holding: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    @staticmethod
    def _generate_recommendations(holdings: List[Dict[str, Any]], sector_allocation: Dict[str, float]) -> List[str]:
        ...
        for sector, allocation in sector_allocation.items():
            if allocation > 30:
                recommendations.append(f"High concentration in {sector} sector ({allocation:.1f}%). Consider diversification.")
        
        # Check individual holdings
        for holding in holdings:
            symbol = holding["symbol"]
            gain_loss_pct = holding["gain_loss_pct"]
            momentum = holding.get("technical_indicators", {}).get("momentum", "neutral")
            ...
            if gain_loss_pct > 50 and momentum == "overbought":
                recommendations.append(f"{symbol}: Consider taking partial profits (up {gain_loss_pct:.1f}%, overbought)")
            
            # Stop loss recommendations
            elif gain_loss_pct < -20 and trend in ["downtrend", "strong_downtrend"]:
                recommendations.append(f"{symbol}: Consider stop loss (down {abs(gain_loss_pct):.1f}%, downtrend)")
            
            # Buy more recommendations
            elif momentum == "oversold" and trend == "uptrend":
                recommendations.append(f"{symbol}: Potential buying opportunity (oversold in uptrend)")
